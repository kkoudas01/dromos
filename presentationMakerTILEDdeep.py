...
import re  # Προσθήκη αυτής της γραμμής
...
def natural_sort_key(filename):
    """
    Βοηθητική συνάρτηση για φυσική ταξινόμηση (natural sort).
    Μετατρέπει το filename σε λίστα με μέρη, όπου οι αριθμοί μετατρέπονται σε int.
    """
    # Χωρίζουμε το filename σε μέρη: κείμενο και αριθμοί
    parts = re.split(r'(\d+)', filename)
    # Μετατρέπουμε τα αριθμητικά μέρη σε int (αφήνουμε τα μη αριθμητικά ως strings)
...
def create_tiled_presentation():
    ...
    try:
        # Λήψη όλων των αρχείων εικόνων και ταξινόμησή τους με φυσική σειρά
        image_files = [f for f in os.listdir(input_dir) if f.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp'))]
        # Ταξινόμηση με βάση την φυσική σειρά (natural sort)
