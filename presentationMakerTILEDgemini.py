...
import re  # <--- ΝΕΟ: Απαραίτητο για τη σωστή ταξινόμηση αριθμών
...
def create_tiled_presentation():
    ...
    # --- ΔΙΟΡΘΩΣΗ: Φυσική ταξινόμηση αρχείων ---
    try:
        # 1. Βρίσκουμε όλα τα αρχεία (αταξινόμητα)
        files_unsorted = [f for f in os.listdir(input_dir) if f.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp'))]
        
        # 2. Συνάρτηση που σπάει το όνομα σε κείμενο και αριθμούς
        def natural_sort_key(s):
            # Χωρίζει το string όπου βρει αριθμούς.
            # Μετατρέπει τους αριθμούς σε int για σωστή σύγκριση.
            return [int(text) if text.isdigit() else text.lower() for text in re.split(r'(\d+)', s)]
            
        # 3. Ταξινομούμε τη λίστα χρησιμοποιώντας το κλειδί
